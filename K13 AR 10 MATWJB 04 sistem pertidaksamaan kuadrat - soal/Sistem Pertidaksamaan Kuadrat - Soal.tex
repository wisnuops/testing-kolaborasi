...
\section{Latihan Soal - PG}
	...
	\begin{enumerate}		
		...
		\item Daerah yang diarsir pada gambar berikut ini merupakah himpunan penyelesaian dari sistem pertidaksamaan ....	\\
		
			\begin{tikzpicture}[scale=0.4]
				\draw[->] (-6, 0) -- (12, 0) node[above] {$x$};
				\draw[->] (0, -3) -- (0, 8) node[left] {$y$};
				\draw[domain=-4:10, smooth, variable=\x] plot ({\x},{(2/9)*(\x)*(\x-6)});
				...
				\draw[dashed] (3, 0) -- (3, -2);
				\draw[dashed] (0, -2) -- (3, -2);
				\path[fill=orange, fill opacity=0.5] (6.51, {-6.51/2 + 4}) to [out=60, in=-110] (10, {2/9 * 10 * (10-6)}) -- (-4, {2/9 * (-4) * (-4-6)}) -- (-2.76, {-(-2.76/2) + 4}) -- (6.51, {-6.51/2 + 4});
			\end{tikzpicture}
		\begin{enumerate}[(A)]
			\item $y \geq 4{,}5x^2 + 27x$ dan $x + 2y \leq 8$
			\item $y \leq 4{,}5x^2 + 27x$ dan $x + 2y \leq 8$
			\item $y \leq 4{,}5x^2 + 27x$ dan $x + 2y \geq 8$
			\item $y \geq 4{,}5x^2 + 27x$ dan $x + 2y \geq 8$
			\item $y \leq 4{,}5x^2 + 27x$ dan $x + 2y \leq 8$
		\end{enumerate} 
		
		Note: Kalau arsirannya nggak transparan, buka file \verb|transparency.pdf|
		...
		\end{enumerate}
		\item Salah satu penyelesaian dari sistem pertidaksamaan:	\\
			\[\left\{\begin{array}{l}
					y < x^2 + 3x + 2	\\
					y \leq 9 - x^2	
				\end{array}\right.\]
